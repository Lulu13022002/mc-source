...
class McSource : Plugin<Project> {
    ...
    private fun registerSidedTasks(ext: McSourceExtension, project: Project, manifest: GameManifest) {
        ...
        val downloadJar = project.tasks.register<DownloadFile>("download${jarId.capitalized()}") {
            group = TASK_GROUP
        ...
        if (jar == JarType.SERVER && !preBundler) { // extract server jar from bundler
            val extractServerJar = project.tasks.register<ExtractServerJar>("extractServerJar") {
                group = TASK_GROUP
        ...
        val minecraft = project.configurations.register("minecraft") {
            isTransitive = false
        ...
        if ((mappingFile != null && (!customJarSet || !remapper.isEmpty)) || mappingUrlOverride != null || preMojMap) {
            val remapJar = project.tasks.register<RemapJar>("remapJar") {
                group = TASK_GROUP
            ...
            if (mappingFile != null || mappingUrlOverride != null) {
                val downloadMapping = project.tasks.register<DownloadFile>("download${jarId.capitalized()}Mappings") {
                    group = TASK_GROUP
        ...
        val decompileJar = project.tasks.register<DecompileJar>("decompile${jarId.capitalized()}") {
            group = TASK_GROUP
        ...
        if (unpickDefinitions.files.size == 1) {
            val unpickJar = project.tasks.register<UnpickJar>("unpickJar") {
                group = TASK_GROUP
                input = remappedJar
